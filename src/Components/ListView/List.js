...
class MoviesList extends React.Component {
    ...
    getMoviesList = async (query = 'batman') => {
        try {
            ...
            const response = await fetch(`http://www.omdbapi.com/?s=${query}&apikey=9ad9524c`)
    ...
    updateMovies = async (query = '') => {
        this.setState({query})
        const data = await this.getMoviesList(query);
        this.setState({movies: data.Search || []})
    }
    ...
    render() {
        return (
            <View style={{flex: 1}}>
                <View style={{height: 40, backgroundColor: '#424242'}}>
                    <TextInput value={this.state.query} onChangeText={this.updateMovies}
                               placeholder="search a Movie batman"
                               placeholderTextColor="#FF6D00"
                               underlineColorAndroid="#FF6D00"
                               style={{color:"#fff"}}
                    />
                </View>
                <ScrollView style={{backgroundColor:'#424242'}}>
                    {
